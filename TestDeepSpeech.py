...
# NB: modified by BP from deepspeech team
# this adapts the code from deepspeech.py to run just testing - it will print out a kaldi 
# compatible output script. Still need more cleanup to make the code much more tenable to 
# maintain
...
tf.app.flags.DEFINE_string  ('result',         'result.txt',            'output file to write result to')
tf.app.flags.DEFINE_boolean ('show_decoding',    False,        'wether to test the network')
...
def get_tower_results(batch_set, optimizer):
    ...
    * The mean edit distance of the outcome averaged over the whole batch ``mean_edit_distance``
    ...
    result tuple is (uttids,labels,decodings,distances,total_losses)
    '''
    ...
    return (tower_uttids, tower_labels, tower_decodings, tower_distances, tower_total_losses), \
           tower_gradients, \
...
# Helpers
# =======
def write_result(results_tuple):
    print ("WRITING RESULT");
    items = list(zip(*results_tuple))
    for uttid, label, decoding, distance, loss in items:
        if FLAGS.show_decoding: print ("%s %s" % (uttid,decoding))
        print ("%s %s" % (uttid,decoding),file=fout)
    print ("DONE WRITING");
...
def collect_results(results_tuple, returns):
    ...
    for i in range(len(available_devices)):
        # Collect the utterance ids
        results_tuple[0].extend(returns[0][i])

        # Collect the labels
        results_tuple[1].extend(sparse_tensor_value_to_texts(returns[1][i]))

        # Collect the decodings - at the moment we default to the first one
        results_tuple[2].extend(sparse_tensor_value_to_texts(returns[2][i][0]))

        # Collect the distances
        results_tuple[3].extend(returns[3][i])

        # Collect the losses
        results_tuple[4].extend(returns[4][i])
...
def test(server=None):
    ...
    # Read all data sets
    data_sets = read_data_sets(FLAGS.test_files.split(','),
